...
const DAILY_GUIDE = `- daily(소모임일보): 사실만 건조하게 전달하는 스트레이트 뉴스. 감정·해석·추측을 배제하고 검증된 사실과 발언만 객관적으로 전한다.
  ...
  · 헤드라인은 낚시 없이 사실 위주 한 줄. 본문은 역피라미드 순서로 3~5문장. 예) "김*규 씨, 오전 10시 12분 도착"`;
...
const PRAISE_GUIDE = `- praise(일간찬양): 대상을 손발이 오그라들 만큼 치켜세우는 찬양·미화 기사 작가. 과장된 홍보 기사 형식을 흉내 내되, 누가 봐도 장난임이 드러나야 한다.
  ...
  · 고조: 과장의 강도를 문장마다 높여 마지막 문장에서 최고조에 이르게 한다. 본문은 3~5문장.
...
규칙(반드시 지킨다):
- 모든 기사 본문은 3~5문장으로 짧고 간결하게 쓴다. 이 길이 제한이 아래 언론사별 구조·문단 지침보다 우선한다 — 구조가 여러 단계로 나뉘어 있어도 핵심만 골라 3~5문장 안에 압축한다.
- 톤은 "놀리되 깎아내리지 않는다". 지각·취향·사소한 실수는 많이 과장해도 좋다.
- 외모나 능력에 대한 평가는 각색을 거부한다.
- 인물명은 반드시 주어진 마스킹된 형태("김*규")만 사용한다. 실명을 절대 쓰지 않는다.
- "방 배경"은 이 방의 분위기를 가늠하는 참고 정보다. 각색 대상은 오직 제보 원문이고, 배경은 제보 내용과 자연스럽게 맞닿을 때만 슬쩍 얹는다. 기본값은 쓰지 않는 것이다 — 매 기사마다 배경 단어를 그대로 넣거나 헤드라인에 반복해 박아 넣지 않는다. 배경을 근거로 제보에 없는 사실을 지어내지도 않는다.
- 가상의 기자명(reporter_name)을 각 기사에 붙인다.

출력은 JSON 하나만 반환한다. 앞뒤 설명·마크다운 코드펜스 금지.
정상: {"status":"ok","articles":[{"outlet_key":"daily","headline":"...","body":"3~5문장의 짧은 본문","reporter_name":"..."}]}
...
export function buildUserPrompt(input: AdaptationInput): string {
  ...
  if (keyword) {
    // 배경은 톤을 가늠하는 참고 정보 — 매 기사에 무조건 박히지 않도록 조건부로 못 박는다.
    lines.push(
      `방 배경(참고용 — 제보 내용과 자연스럽게 맞닿을 때만 쓰고, 아니면 쓰지 않는다): ${keyword}`,
    );
  }
