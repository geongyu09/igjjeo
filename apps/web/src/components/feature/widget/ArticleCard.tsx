...
function Photo({ imageUrl, headline, small }: { imageUrl?: string; headline: string; small?: boolean }) {
  if (imageUrl) {
    // eslint-disable-next-line @next/next/no-img-element -- 외부 저장소(Supabase Storage) 이미지, 크기 미정 단계
    return <img className={small ? styles.thumb : styles.photo} src={imageUrl} alt={headline} />;
  }
  return (
    <div className={small ? styles.thumbPlaceholder : styles.photoPlaceholder} aria-hidden>
      {!small && "사진"}
    </div>
  );
}
...
export function ArticleCard({
  ...
  if (variant === "hero") {
    return (
      <article className={rootClassName} data-variant="hero" {...interactiveProps}>
  ...
  if (variant === "list") {
    return (
      <article className={rootClassName} data-variant="list" {...interactiveProps}>
  ...
  if (variant === "compact") {
    return (
      <article className={rootClassName} data-variant="compact" {...interactiveProps}>
        <PublisherBadge outlet={outlet} variant="text" />
        <h3 className={styles.listHeadline}>{headline}</h3>
        <div className={styles.monoMeta}>
          👀 {viewCount ?? 0} · 댓글 {commentCount ?? 0}
  ...
  return (
    <article className={rootClassName} data-variant="large" {...interactiveProps}>
      ...
      <div className={styles.footer}>
        ...
        {reporterLabel && <span className={styles.reporter}>제보 {reporterLabel}</span>}
      </div>
    </article>
  );
}
