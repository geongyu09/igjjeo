...
describe("PublisherRadioGroup", () => {
  ...
  it("선택된 언론사만 aria-checked=true다", () => {
    ...
    expect(screen.getByRole("radio", { name: "데일리쇼크" })).toHaveAttribute(
      "aria-checked",
      "true",
    );
    expect(screen.getByRole("radio", { name: "소모임일보" })).toHaveAttribute(
      "aria-checked",
      "false",
    );
  });
  ...
  it("선택된 항목만 tab 순서에 들어가고, 선택이 없으면 첫 항목이 들어간다", () => {
    const { rerender } = render(
      <PublisherRadioGroup value="shock" onChange={() => {}} />,
    );
    expect(screen.getByRole("radio", { name: "데일리쇼크" })).toHaveAttribute(
      "tabindex",
      "0",
