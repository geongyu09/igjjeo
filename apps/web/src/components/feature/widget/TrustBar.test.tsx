...
describe("TrustBar", () => {
  ...
  it("total 기준으로 막대 너비를 계산한다", () => {
    const { container } = render(<TrustBar admitCount={7} reallyCount={2} total={10} />);
    expect(container.querySelector<HTMLElement>('[data-part="admit"]')!.style.width).toBe("70%");
    expect(container.querySelector<HTMLElement>('[data-part="really"]')!.style.width).toBe("20%");
  });

  it("total이 없으면 두 카운트의 합을 기준으로 한다", () => {
    const { container } = render(<TrustBar admitCount={3} reallyCount={1} />);
    expect(container.querySelector<HTMLElement>('[data-part="admit"]')!.style.width).toBe("75%");
    expect(container.querySelector<HTMLElement>('[data-part="really"]')!.style.width).toBe("25%");
  });
  ...
  it("반응이 없으면 막대 너비는 0이다", () => {
    ...
    expect(container.querySelector<HTMLElement>('[data-part="admit"]')!.style.width).toBe("0%");
  });
});
