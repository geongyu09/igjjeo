...
const KIND_BADGE: Record<Exclude<CorrectionKind, "original">, { label: string; variant: "accent" | "accent-strong" }> = {
      ...
      {items.map((item, index) => (
        <li key={index} className={styles.node} data-kind={item.kind}>
          <div className={styles.head}>
            {item.kind !== "original" && (
              <CornerUpLeft size={13} className={styles.replyIcon} aria-hidden />
            )}
            <PublisherBadge outlet={item.outlet} />
            {item.kind !== "original" && (
              <Badge variant={KIND_BADGE[item.kind].variant}>{KIND_BADGE[item.kind].label}</Badge>
            )}
          </div>
