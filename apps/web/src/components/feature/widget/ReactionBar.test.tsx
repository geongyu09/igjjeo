...
describe("ReactionBar", () => {
  it("반응 4종을 카운트와 함께 렌더링한다", () => {
    ...
    expect(screen.getByRole("button", { name: /진짜\? 2/ })).toBeInTheDocument();
  ...
  it("내가 누른 반응만 aria-pressed=true다", () => {
    ...
    expect(screen.getByRole("button", { name: /인정/ })).toHaveAttribute("aria-pressed", "true");
    expect(screen.getByRole("button", { name: /충격/ })).toHaveAttribute("aria-pressed", "false");
  });
