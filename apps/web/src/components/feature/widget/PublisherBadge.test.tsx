...
describe("PublisherBadge", () => {
  ...
  it("outlet을 data 속성으로 노출한다", () => {
    ...
    expect(screen.getByText("스터디경제")).toHaveAttribute("data-outlet", "economy");
  });

  it("기본 variant는 soft다", () => {
    render(<PublisherBadge outlet="daily" />);
    expect(screen.getByText("소모임일보")).toHaveAttribute("data-variant", "soft");
  });
  ...
  it("solid·text variant를 지원한다", () => {
    ...
    expect(screen.getByText("데일리쇼크")).toHaveAttribute("data-variant", "solid");
    expect(screen.getByText("소모임일보")).toHaveAttribute("data-variant", "text");
  });
});
