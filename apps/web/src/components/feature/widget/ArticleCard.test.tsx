...
describe("ArticleCard", () => {
  it("헤드라인과 언론사를 렌더링한다", () => {
    render(<ArticleCard outlet="shock" headline="【단독】 상습 지각, 이대로 괜찮은가" />);
  ...
  it("기본 variant는 large다", () => {
    ...
    expect(screen.getByRole("article")).toHaveAttribute("data-variant", "large");
  });
  ...
  it("large 기본은 사진 플레이스홀더를 보여주고, hidePhoto면 생략한다", () => {
    const { rerender } = render(<ArticleCard outlet="shock" headline="제목" excerpt="발췌" />);
    expect(screen.getByText("사진")).toBeInTheDocument();
    rerender(<ArticleCard outlet="shock" headline="제목" excerpt="발췌" hidePhoto />);
    expect(screen.queryByText("사진")).not.toBeInTheDocument();
  });

  it("hero는 badge 슬롯을 렌더링한다", () => {
    render(
      <ArticleCard variant="hero" outlet="shock" headline="제목" badge={<span>오늘 가장 뜨거운</span>} />,
    );
  ...
  it("list는 시간 라벨을 메타에 보여준다", () => {
    render(
      <ArticleCard variant="list" outlet="economy" headline="제목" viewCount={7} commentCount={2} timeLabel="1시간" />,
    );
