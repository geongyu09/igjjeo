...
describe("SegmentedControl", () => {
  it("모든 옵션을 radio로 렌더링한다", () => {
    render(
      <SegmentedControl options={OPTIONS} value="pick" onChange={() => {}} />,
    );
  ...
  it("현재 값만 aria-checked=true다", () => {
    ...
    expect(screen.getByRole("radio", { name: "내가 고르기" })).toHaveAttribute(
      "aria-checked",
      "true",
    );
    expect(screen.getByRole("radio", { name: "무작위 3곳" })).toHaveAttribute(
      "aria-checked",
      "false",
    );
  });
  ...
  it("옵션을 클릭하면 해당 값으로 onChange가 호출된다", async () => {
    ...
    render(
      <SegmentedControl options={OPTIONS} value="pick" onChange={onChange} />,
    );
